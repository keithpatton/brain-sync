...
class CircuitBreaker:
    ...
    def record_failure(self) -> None:
        ...
        if self._failures >= self.failure_threshold:
            self._opened_at = time.monotonic()
            log.warning("Circuit breaker opened after %d consecutive failures", self._failures)
...
async def async_retry(
    ...
    **kwargs: object,
    ...
    raise RuntimeError(f"Retry attempts exhausted ({max_retries + 1} attempts): {last_error}")
