...
- **Read** `knowledge/` content and managed summaries via MCP tools
- **Read** managed summaries via `.brain-sync/insights/` and journals via `.brain-sync/journal/`
- **Do not write** to managed `.brain-sync/` files — they are regenerated by brain-sync when knowledge changes
