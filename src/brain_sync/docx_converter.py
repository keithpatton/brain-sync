...
def _collect_text_between(start_elem, end_elem) -> str:
    ...
    for elem in body.iter():
        ...
        if elem is end_elem:
            break
        if collecting and elem.tag == f"{{{_W_NS}}}t":
            if elem.text:
...
def _extract_comments_from_doc(doc: Document) -> list[dict]:
    ...
    for comment_elem in comments_xml.findall(f"{{{_W_NS}}}comment"):
        ...
        date_display = ""
        if date_str:
            # Typical format: 2026-03-05T10:30:00Z
    ...
    # Extract annotated text ranges from document body
    ...
    for cid, info in comments_by_id.items():
        ...
        if starts and ends:
            ...
            info["annotated_text"] = annotated[:200]  # Truncate long ranges

    # Sort by date, then author
    result = sorted(
        comments_by_id.values(),
        key=lambda c: (c.get("date", ""), c.get("author", "")),
    )
    return result
...
def extract_comments(docx_path: Path) -> str | None:
    ...
    Returns markdown string with comment entries, or None if no comments.
    """
...
def _para_to_markdown(para) -> str:
    ...
    if para._element.find(f".//{{{_W_NS}}}numPr") is not None:
        ...
        indent_str = "  " * indent
        # Simple heuristic: use "- " for all lists (no reliable way to distinguish)
        prefix = indent_str + "- "
...
def docx_to_markdown(docx_path: Path) -> str:
    ...
    Body text is extracted via python-docx with basic style mapping.
    For higher fidelity, use Google Docs markdown export + extract_comments().
    """
    doc = Document(str(docx_path))
    parts: list[str] = []
...
def append_comments_to_markdown(md_path: Path, docx_path: Path) -> bool:
    ...
    Replaces existing ## Comments section if present (idempotent).
    Returns True if comments were added/updated.
    """
    ...
    # Strip existing ## Comments section if present
    # Match "---\n\n## Comments\n" or just "## Comments\n" at the end
    md_content = re.sub(
        r"\n---\n\n## Comments\n.*",
        "",
