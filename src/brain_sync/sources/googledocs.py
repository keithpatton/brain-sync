...
def _gcloud_cmd() -> str:
    ...
    if cmd is None:
        raise FileNotFoundError(
            "gcloud not found on PATH. Install from https://cloud.google.com/sdk"
        )
    return cmd
...
async def _get_access_token() -> str:
    ...
    try:
        ...
        proc.kill()
        raise FetchError("gcloud auth print-access-token timed out")

    if proc.returncode != 0:
        raise FetchError(
            f"gcloud auth failed (exit {proc.returncode}): {stderr.decode().strip()}"
        )
...
async def googledocs_fetch(doc_id: str, client: httpx.AsyncClient) -> str:
    ...
    try:
        response = await client.get(
            url, headers=headers, follow_redirects=True, timeout=HTTP_TIMEOUT
        )
