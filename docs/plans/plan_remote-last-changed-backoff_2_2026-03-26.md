...
Planning revision addressing
`docs/plans/plan_remote-last-changed-backoff_review_1_2026-03-26.md`.

This revision resolves the review blockers by:

- fixing the semantic contract of `remote_last_changed_utc`
- fixing the authoritative rollout/reset mechanism for `sync_polling`
- treating the work as an explicit runtime DB schema change with the required
  versioning, compatibility, and migration-test trail

This artifact does not change product behavior. It is intended to be
approval-ready under the `docs/plans/` workflow.
