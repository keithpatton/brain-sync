# plan_source-lifecycle-hardening_8
...
`docs/plans/plan_source-lifecycle-hardening_7_2026-03-20.md` for review
...
13. The old doctor destructive entrypoints are removed or rejected with a
   ...
   surfaces.
14. Registered-source `move source` is a first-class lifecycle command owned
    ...
    mutation paths.
15. Administrative listing surfaces expose lifecycle state explicitly for
    every registered source, and missing sources remain visible in those
    listings.
16. CLI `list` prints `State: <knowledge_state>` for every returned source,
    including `State: missing` for missing registered sources; MCP
    `brain_sync_list` returns the matching `knowledge_state` field.
17. Missing sources retain runtime lifecycle coordination until explicit
    finalization or rediscovery/materialization.
18. `stale` still forbids unchanged-source fast paths until successful full
    re-materialization.
19. `sync/daemon.py` no longer depends on `application.*` lifecycle helpers.
20. Architecture-boundary tests enforce zero production `sync -> application`
    imports and name the approved lifecycle orchestrator modules explicitly.
21. Architecture-boundary tests reserve the named `BrainRepository` lifecycle
    ...
    `sync/lifecycle.py`.
22. `sync/lifecycle_policy.py` remains IO-free and is guarded by tests.
23. Brain, runtime, architecture, glossary, schema, versioning,
