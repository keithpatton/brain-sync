...
Approved: 2026-03-18T11-00-51
Approved by: codex-gpt-5

Notes:
- The plan makes the unchanged-source Confluence contract explicit: page-version equality governs page/comments freshness, while attachment repair remains a separate required maintenance path.
- The portable-manifest gate is specific enough to prevent accidental Brain Format `1.0` drift, because any durable field change must bring versioning, compatibility, schema-doc, and compatibility-test work in the same change set.
- The sequencing and acceptance criteria are concrete enough to keep the startup fast-path fix separate from the later timestamp-semantics review and to avoid implementer guesswork about attachment repair or partial-state merging.
...
`docs/plans/plan_source-fetch-hardening_2_2026-03-18.md` as the authoritative
