...
Approved: 2026-03-22T11-47-48
Approved by: Codex GPT-5 (reviewer/approver)

Notes:

- revision 5 resolves the prior approval blockers by choosing one coherent shipped contract: single-call explicit finalization for already-missing sources, no `pending_confirmation`, and a released `0.7` runtime schema of `v29`
- scope remains acceptably bounded to explicit finalization, result shaping, and the required runtime-schema/versioning/doc/test convergence rather than reopening the broader v2 lifecycle-hardening package
- implementation should make the `v29` treatment of `local_missing_first_observed_utc` and `local_missing_last_confirmed_utc` explicit in code, migration tests, and owning docs so the schema does not retain unexplained residue
...
for this follow-up topic.

This revision is approved by this artifact for implementation.
