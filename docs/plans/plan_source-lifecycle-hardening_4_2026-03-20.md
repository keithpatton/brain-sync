# plan_source-lifecycle-hardening_4
...
`docs/plans/plan_source-lifecycle-hardening_3_2026-03-20.md` for review
...
- add boundary tests that enforce:
  ...
  - doctor may continue to use non-lifecycle `BrainRepository` reads and
    ...
    source lifecycle mutation method
...
14. Architecture-boundary tests reserve the named `BrainRepository` lifecycle
    mutation methods to `sync/lifecycle.py` and `sync/finalization.py`.
15. `sync/lifecycle_policy.py` remains IO-free and is guarded by tests.
16. Brain, runtime, architecture, glossary, schema, versioning,
