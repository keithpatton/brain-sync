# plan_source-lifecycle-hardening_6
...
`docs/plans/plan_source-lifecycle-hardening_5_2026-03-20.md` for review
...
10. `sync/lifecycle.py` and `sync/finalization.py` are the only production
    ...
    transitions, path repair, folder-move manifest rewrites, explicit remove,
...
19. Architecture-boundary tests reserve the named `BrainRepository` lifecycle
    ...
    transitions, path repair, folder-move manifest rewrites, explicit remove,
    and explicit finalization to `sync/lifecycle.py` and
    `sync/finalization.py`.
