...
class TreeNode:
    ...
    child_folder_count: int         # direct child knowledge areas
...
same JSON field set and naming:
...
No additional top-level or nested fields are part of the phase-1 contract.
...
The MCP response must conform exactly to the JSON contract above.
...
CLI `--json` output must conform exactly to the same JSON contract as MCP.
...
4. Every node in MCP and CLI `--json` output contains exactly:
   - `path`
   - `name`
   - `depth`
   - `child_folder_count`
...
9. CLI command `brain-sync tree` exists with:
   - parser wiring
   - handler wiring
   - `__main__` dispatch wiring
   - default human-readable output
   - `--json` output
10. MCP tool `brain_sync_tree` exists and returns the exact same JSON schema as
    CLI `--json`.
11. Tests prove:
   - application behavior
   - MCP surface behavior
   - CLI `--json` surface behavior
12. The implementation is read-only: no mutations to the brain, runtime DB,
    or filesystem.
