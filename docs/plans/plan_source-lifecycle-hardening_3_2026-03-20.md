# plan_source-lifecycle-hardening_3
...
`docs/plans/plan_source-lifecycle-hardening_2_2026-03-20.md` for review
...
8. `README.md`
   ...
   - remove or revise user-facing references to destructive doctor cleanup if
     they still appear
   - update MCP tool documentation if user-facing tables currently describe the
     old destructive doctor surface
...
`sync_polling` remains active-polling-only. Missing sources may leave
`sync_polling`, but they must remain in `source_lifecycle_runtime` until:
...
- refuse to finalize when local runtime coordination is absent, insufficient,
  or stale
...
- add boundary tests that enforce:
  ...
  - `src/brain_sync/sync/lifecycle.py` and
    `src/brain_sync/sync/finalization.py` are the only production lifecycle
    orchestrator modules allowed to drive portable source lifecycle mutation
    through `brain.repository`
  - `src/brain_sync/sync/pipeline.py` may materialize content through
    `brain.repository`, but it must not own lifecycle state transitions
...
10. `sync/lifecycle.py` and `sync/finalization.py` are the only production
    sync modules allowed to orchestrate lifecycle-state mutation through
    `brain.repository`.
...
7. Doctor is non-destructive by default and does not bypass lifecycle APIs.
8. The old doctor destructive entrypoints are removed or rejected with a
   directed migration hint; they do not survive as independent destructive
   surfaces.
9. Missing sources remain visible in administrative listing surfaces.
10. Missing sources retain runtime lifecycle coordination until explicit
    finalization or rediscovery/materialization.
11. `stale` still forbids unchanged-source fast paths until successful full
    re-materialization.
12. `sync/daemon.py` no longer depends on `application.*` lifecycle helpers.
13. Architecture-boundary tests enforce zero production `sync -> application`
    imports and name the approved lifecycle orchestrator modules explicitly.
14. `sync/lifecycle_policy.py` remains IO-free and is guarded by tests.
15. Architecture, glossary, schema, versioning, and compatibility docs are all
    updated consistently with the implementation.
