...
Approved: 2026-03-26T09-13-57
Approved by: codex-gpt-5

Notes:

- the plan stays narrowly scoped to reordering only within the already-due polling batch
- no portable brain contract, runtime schema, or scheduling-authority change is implied by this work
- the acceptance criteria and verification requirements are specific enough to preserve missing-source exclusion and post-poll regen sequencing
...
`docs/plans/PLAN_poll-batch-ordering.md` as the authoritative implementation
handoff.

This approval covers a narrow daemon-loop behavior change: reorder the
already-due batch so that higher-value lifecycle states are checked first while
preserving the current scheduler, persistence, and regen boundaries.
...
If execution discovers a material contradiction, higher-authority rules
