# APPROVED: Structural Health Phase 1 (brain_sync_tree)

Approved: 2026-03-27T09:46:50+13:00
Approved by: Codex

Attestation:

- This revision is approved for implementation.
- The semantic knowledge-area tree contract, `_core/` optionality, synced-file grouping by parent of `knowledge_path`, managed-file exclusion rule, sparse wire defaults, and MCP/CLI `--json` parity requirements now align with the authoritative docs and current code.
- `src/brain_sync/main.py` does not exist in this repository; the corresponding CLI dispatch surface reviewed for this plan is `src/brain_sync/__main__.py`.
...
- include `_core/` only when it exists and qualifies as a semantic knowledge
  area under the same filesystem-driven rules as any other node
...
- do not synthesize `_core/` when it is absent
...
2. The tree includes the root `knowledge/` node and any `_core/` node only
   when `_core/` exists and qualifies as a semantic knowledge area.
3. The tree excludes directories that contain neither readable files nor
   descendant knowledge areas.
4. The tree does not synthesize `_core/` when it is absent.
5. MCP and CLI `--json` return the exact same sparse JSON schema.
6. Every node in MCP and CLI `--json` output always includes:
   - `path`
   - `depth`
7. Every other node field follows the sparse default semantics defined in this
   plan.
8. The only allowed JSON node fields in phase 1 are:
   - `path`
   - `depth`
   - `child_folder_count`
   - `manual_file_count`
...
9. `manual_file_count` excludes synced managed markdown files.
10. Synced-file counts are broken down by `awaiting`, `materialized`, `stale`,
...
11. `insights.artifact_count` excludes `insight-state.json`.
12. `journals.entry_count`, `first_entry_date`, and `last_entry_date` are
    correct for current journal layout.
13. CLI command `brain-sync tree` exists with:
   - parser wiring
   - handler wiring
...
15. The implementation is read-only: no mutations to the brain, runtime DB,
    or filesystem.
