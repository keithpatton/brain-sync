# plan_source-lifecycle-hardening_2

## Status

Plan revision 2.

This revision supersedes `docs/plans/PLAN_source-lifecycle-hardening.md` for
...
- refuse to finalize when local runtime coordination is absent or stale
...
- add boundary tests that enforce:
  - no `sync -> application` imports outside closed orchestration exceptions
  - no IO-layer imports inside `sync/lifecycle_policy.py`
  - no direct source-manifest lifecycle mutation outside `brain.repository`
    and the approved lifecycle orchestrator path
...
5. Explicit `reconcile` is non-destructive with respect to missing-source
   finalization.
6. Doctor is non-destructive by default and does not bypass lifecycle APIs.
7. Missing sources remain visible in administrative listing surfaces.
8. Missing sources retain runtime lifecycle coordination until explicit
   finalization or rediscovery/materialization.
9. `stale` still forbids unchanged-source fast paths until successful full
   re-materialization.
10. `sync/daemon.py` no longer depends on `application.*` lifecycle helpers.
11. `sync/lifecycle_policy.py` remains IO-free and is guarded by tests.
12. Architecture, glossary, schema, versioning, and compatibility docs are all
...
- architecture-boundary tests removing the `sync -> application` exception and
  guarding the new split
