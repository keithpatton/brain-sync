# plan_runtime-release-alignment_3
...
`docs/plans/plan_runtime-release-alignment_2_2026-03-18.md` for review
purposes and addresses the findings in
`docs/plans/plan_runtime-release-alignment_2_review_1_2026-03-18.md`.
...
- determining the exact keep/revert/defer disposition before any code changes
...
The output of this plan is a narrowed, reviewable decision artifact. It is not
...
### Consequence for disputed runtime tables

This revision closes the prior loophole:

- if a disputed machine-local runtime table proves to be correctness-critical
  for startup reconcile, watcher enqueue, rename/move propagation, query-index
  invalidation, or any other retained workflow, that table fails retention in
  this narrowing effort
- such a table must then be:
  - reverted
...
It may not remain in scope as a "correctness-critical runtime cache."
...
### 3. Existing tests prove rebuildability, but not enough disputed-surface safety
...
- stale rows in disputed tables cannot suppress required work
- the disputed surfaces remain advisory in every consuming workflow
...
- explicitly accepted as remaining in scope after the narrowing review

### Provisional disputed surfaces

These remain under dispute:
...
- if any proves correctness-critical, it fails retention in this narrowing
  effort

## Pre-Release Rollback and Runtime-Shape Policy

This revision now forces Phase 3 to choose one exact runtime-shape strategy.
...
1. Keep current `v25` runtime shape and retain disputed tables
Condition:
those tables survive review and validation

2. Keep runtime schema label `v25`, but treat pre-narrowing local DBs as
unsupported and rebuild on explicit detection
Condition:
the narrowed branch no longer trusts the disputed table set, but does not bump
runtime schema again during pre-release narrowing
...
- any disputed runtime table that proves correctness-critical fails retention in
  this narrowing effort
...
- inventory the exact disputed surface
...
- determine whether each disputed runtime surface is truly advisory

Actions:

- classify each disputed table as:
...
- one disposition row per disputed table with:
  - owner
  - authoritative input
  - rebuild source
...
- if a disputed table is classified as correctness-critical, it cannot remain
  as a retained runtime-only surface in this plan

Exit criteria:

- every disputed table has an explicit authority disposition
...
- only those disputed surfaces that survive the post-Phase-3 review
...
- closes the correctness-critical retention loophole for disputed runtime
  tables
