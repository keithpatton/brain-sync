# plan_source-lifecycle-hardening_5
...
`docs/plans/plan_source-lifecycle-hardening_4_2026-03-20.md` for review
...
- `canonical_id`
- `finalized: true`
- the terminal result state `finalized`
...
- add boundary tests that enforce:
  ...
  - `src/brain_sync/sync/pipeline.py` may materialize content through
    `BrainRepository.materialize_markdown()` and normal non-lifecycle
    directory/file helpers, but it must not call any reserved source lifecycle
    mutation method
...
10. `sync/lifecycle.py` and `sync/finalization.py` are the only production
    ...
    transitions, path repair, explicit remove, and explicit finalization.
...
9. CLI and MCP finalization surfaces share the same terminal result states:
   ...
   `not_found`.
10. Doctor is non-destructive by default and does not bypass lifecycle APIs.
11. The old doctor destructive entrypoints are removed or rejected with a
   directed migration hint; they do not survive as independent destructive
   surfaces.
12. Missing sources remain visible in administrative listing surfaces.
13. Missing sources retain runtime lifecycle coordination until explicit
    finalization or rediscovery/materialization.
14. `stale` still forbids unchanged-source fast paths until successful full
    re-materialization.
15. `sync/daemon.py` no longer depends on `application.*` lifecycle helpers.
16. Architecture-boundary tests enforce zero production `sync -> application`
    imports and name the approved lifecycle orchestrator modules explicitly.
17. Architecture-boundary tests reserve the named `BrainRepository` lifecycle
    mutation methods for source registration, settings updates, missing
    transitions, path repair, explicit remove, and explicit finalization to
    `sync/lifecycle.py` and `sync/finalization.py`.
18. `sync/lifecycle_policy.py` remains IO-free and is guarded by tests.
19. Brain, runtime, architecture, glossary, schema, versioning,
