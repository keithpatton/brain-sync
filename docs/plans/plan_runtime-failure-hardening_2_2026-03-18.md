# plan_runtime-failure-hardening_2: Runtime Failure Hardening Review Bundle

## Status

Plan revision 2. This artifact supersedes the root plan for review purposes and
addresses the issues raised in
`docs/plans/plan_runtime-failure-hardening_review_1_2026-03-18.md`.

This stage is still planning only. It does **not** approve implementation and
does **not** modify the earlier plan or review artifacts.
...
`application/insights.py` writes portable insight state first and runtime
lifecycle second.

The `insight_state.knowledge_path` uniqueness evidence is still relevant, but
it refers to a historical failure mode. The active workstream must target the
current owner and explain how the historical bug maps to current code.
...
- a migration/legacy path that still needs a narrow fix
- or a stale/obsolete work item that should be reduced to a guardrail rather
...
obvious from the active source/sync/runtime seams.

#### D. Filesystem permission / lock pressure
...
- defining and enforcing explicit conflict contracts for:
  ...
  - `documents.url`
- adding a concrete bounded retry/defer policy for WinError 5 filesystem
...
existing application/repository source lifecycle may persist missing status.
...
- replay of the same normalized path is replay-safe
- runtime lifecycle state may be updated in place via upsert
- when concurrent or repeated callers write different lifecycle fields for the
  same normalized path, the later runtime write wins for lifecycle-only fields
  (`regen_status`, `regen_started_utc`, `owner_id`, `error_reason`)
- this does **not** reopen or override portable state; portable writes remain
  governed by `application/insights.py` and `brain/repository.py`
...
- repeated writes to the same normalized `knowledge_path` do not fail with a
  uniqueness error
- tests cover replay of the same path with changed lifecycle fields
...
- before code changes, identify the live caller that still writes
  `documents.url`, or prove the evidence comes from a legacy/migration-only
  path
- if the failure is legacy-only, land a narrow fix at that path and do not
  reintroduce `documents` as a normal sync/regen write surface
...
|---|---|---|---|---|
| queue-driven regen of one `knowledge_path` | `regen/queue.py` | max 3 retries in the same queue lifetime, using existing 30s / 60s / 120s backoff for classified WinError 5 lock contention | stop retrying in the active queue, record classified failure state, do not keep hot-looping | next watcher event, explicit regen invocation, or next startup reconcile/session |
| portable-brain file replacement / delete during regen | `brain/repository.py` called by regen | no inner unbounded retry loop inside repository; raise a classified lock error to queue/orchestrator | leave portable state unchanged, allow caller to record failure/defer state | queue/orchestrator |
...
- classify WinError 5 / `PermissionError` lock pressure separately from generic
  logic failures
...
- queue/integration tests that show no more than three attempts for one path in
  a single active run
...
- sync/application layer owns translation of that classification into durable
  source lifecycle changes
- persistent missing state must reuse the existing manifest/reconcile missing
  path rather than inventing a second state machine
...
3. Implement Workstream 3 after conflict contracts are explicit, so lock/defer
   handling does not accidentally mask identity bugs.
...
- states a concrete bounded retry/defer policy for Windows lock handling
- attaches Confluence `404` handling to the existing missing-source lifecycle
- tightens evidence anchors with run IDs, adjacent messages, and stderr/shared
  log ownership notes
- leaves no material behavior decision to implementer guesswork

If review feedback identifies further issues, the planner should create another
new plan artifact rather than editing this file in place.
