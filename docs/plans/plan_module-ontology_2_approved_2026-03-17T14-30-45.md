...
Approved: 2026-03-17T14-30-45
Approved by: codex-gpt-5

Notes:
- Revision 2 resolves the prior blocking issue by explicitly splitting current `layout.py` ownership into portable `brain.layout` and machine-local `runtime.paths`.
- The resulting package ontology now keeps the portable and runtime persistence planes separate without relying on an unstated `runtime -> brain` dependency.
- The migration phases, shim rules, and acceptance criteria are now specific enough to guide implementation without leaving the layout split implicit.
...
This revision resolves Review 1 by making the current `layout.py` split
explicit: portable layout and version helpers belong to `brain.layout`, while
machine-local runtime path ownership belongs to `runtime.paths`.
...
- portable layout and version semantics only

Out of scope:

- machine-local runtime DB and daemon state
- machine-local runtime path helpers such as user config directory, runtime DB
  location, and daemon-status file location
...
- runtime path ownership such as `brain_sync_user_dir()`, `runtime_db_path()`,
  and `daemon_status_path()`
...
Normative note:
No machine-local path helper may remain in `brain.layout`. If a helper resolves
or names filesystem state under `~/.brain-sync/`, it belongs to `runtime`.
...
### Layout and path ownership rule

Portable layout helpers may live in `brain.layout`.
Machine-local runtime path helpers must live in `runtime.paths` or
`runtime.config`.

`runtime -> brain` is not an allowed dependency for path ownership. The layout
split must therefore be structural, not merely documentary.
...
| `src/brain_sync/config.py` | `runtime.config` | It owns runtime config file access and configuration loading. |
...
| `src/brain_sync/layout.py` | split: portable layout/version helpers to `brain.layout`; machine-local runtime path helpers to `runtime.paths` | The current file mixes two persistence planes. The revision explicitly requires a split so portable layout stays in `brain` and runtime path ownership moves under `runtime`. |
...
- `layout.py`: mixed across persistence planes today and must be split rather
...
|---|---|---|
| `layout.py` | split into `brain/layout.py` and `runtime/paths.py` | The current file spans both persistence planes. Keeping it whole would either leak runtime ownership into `brain` or force an unplanned `runtime -> brain` dependency. |
...
- split current `layout.py` into portable `brain.layout` and machine-local
  `runtime.paths` before other package moves depend on it
...
9. Current `layout.py` ownership has been resolved by splitting portable layout
   into `brain` and machine-local runtime paths into `runtime`.
10. Top-level resource directories have been absorbed by their owning
   subsystems or removed.
11. Legacy paths survive only as temporary shims with no new logic.
...
4. Does the plan now resolve `layout.py` coherently by splitting portable
   layout from machine-local runtime path ownership?
5. Is `application` the right name and scope for the current `commands/`
   package?
6. Does the plan correctly treat regen as a first-class subsystem?
7. Are mixed modules such as `attachments.py` and `commands/placement.py`
   handled honestly rather than hidden inside vague package names?
8. Does the phased migration minimize long-lived shims and conceptual drift?
