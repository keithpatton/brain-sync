...
Approved: 2026-03-26T10-07-16
Approved by: codex-gpt-5

Notes:

- the revision resolves the prior review findings by fixing the selector contract, CLI outcome mapping, daemon-parity boundaries, and verification expectations
- the plan keeps the implementation narrowly scoped to one direct per-source forced sync path without inline regen, lease override, daemon lifecycle control, or bulk child sync expansion
- the architecture and ownership guidance is specific enough to keep business logic below the CLI layer and avoid a scheduler-trigger hack via `sync_polling` mutation
- no unresolved approval blockers remain
...
`docs/plans/plan_sync-source-command_2_2026-03-26.md` as the authoritative
implementation handoff.

This approval covers a narrow new CLI command for one immediate single-source
sync attempt that works with or without the daemon running while preserving the
existing lifecycle lease, missing-source handling, and runtime-state ownership
boundaries.
...
Implementation should execute from this approved artifact under the
`docs/plans/` workflow.

If execution discovers a material contradiction, a higher-authority rule
conflict, or a missing contract choice that changes requirements, stop and
return the work to planning rather than silently redesigning the feature.
